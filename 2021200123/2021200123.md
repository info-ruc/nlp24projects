...
```
### 4.5 古诗生成模型
最终生成一个五言诗模型和两个表现较好的七言诗模型，我们再次使用测试标题进行测试，测试代码如下：
```python
text_generator("[CLS]春 日 访 王 丞 相", max_length=100, do_sample=True,temperature=1.1,top_k=50)
text_generator("[CLS]江 岸 别 杜 甫", max_length=80, do_sample=True,temperature=1.1,top_k=50)
text_generator("[CLS]送 白 居 易", max_length=80, do_sample=True,temperature=1.1,top_k=50)
text_generator("[CLS]月 见 ", max_length=100, do_sample=True,temperature=1.1,top_k=50)
text_generator("[CLS]与 李 白 会 诗", max_length=100, do_sample=True,temperature=1.1,top_k=50)
print(gen_poem("北山雪X"))
```
五言诗模型（较好的输出结果）：
#### 春 日 访 王 丞 相 
