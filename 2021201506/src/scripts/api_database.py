...
def insert_data():
    ...
    uri_parameters_data = [
        # (api_name, parameter, is_necessary, type, description)
    ...
    request_parameters_data = [
        # (api_name, table_id, parameter, is_necessary, type, description)
        ...
        ("创建云服务器", 3, "allowed_address_pairs", False, "Array ofallow_address_pairobjects", "IP/Mac对列表，详情请参见表4(扩展属性)。约束：IP地址不允许为 “0.0.0.0/0”如果allowed_address_pairs配置地址池较大的CIDR（掩码小于24位），建议为该port配置一个单独的安全组如果allowed_address_pairs为“1.1.1.1/0”，表示关闭源目地址检查开关如果是虚拟IP绑定云服务器，则mac_address可为空或者填写被绑定云服务器网卡的Mac地址。被绑定的云服务器网卡allowed_address_pairs的IP地址填“1.1.1.1/0”。"),
    ...
    cursor.executemany('''
        INSERT INTO uri_parameters (api_name, parameter, is_necessary, type, description)
        VALUES (?, ?, ?, ?, ?)''', uri_parameters_data)

    cursor.executemany('''
        INSERT INTO request_parameters (api_name, table_id, parameter, is_necessary, type, description)
